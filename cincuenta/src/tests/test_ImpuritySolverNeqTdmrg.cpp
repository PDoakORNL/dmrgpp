...
//
// U=0 (no quench, no interaction): tDMRG is exact here (no entanglement
// growth, matching the existing inputU0NeqTdmrg.ain regression input's own
// rationale), so this isolates chaining/restart-mechanics bugs from
...
TEST_CASE("ImpuritySolverNeqTdmrg chained column-0 matches monolithic solve()",
          "[ImpuritySolverNeqTdmrg][Phase1]")
{
	...
	std::cout << "=== Phase 1 chained-vs-monolithic column-0 comparison ===\n";
