...
precision at all three $L_\mathrm{bath}$, both before and after the Fig.\
9 fix (this anchor can't catch an overall scale error since it's
identically zero either way --- see the Fig.\ 9 callout for how the
$E_\mathrm{kin}$ bug was actually caught). No bugs found in the $d(t)$
pipeline itself; suppression with $U$ and the shrinking
...
\begin{table}[H]
  ...
  \begin{tabular}{p{3.1cm}p{3.4cm}p{7.5cm}}
    ...
    $L=4$. Lanczos was seeding incorrectly for $L\ge4$ until the fix
    described in the Fig.\ 9 callout above; now verified correct through
    $L=4$. \\
    \addlinespace
...
\begin{itemize}
  ...
  \item \textbf{C++ Fig.\ 3 parity} --- after fixing the two-time
    Green's-function seeding bug, the production solver's own online
    reconstruction now overlays the Python Cholesky reference essentially
    exactly on full self-consistency runs, eliminating a spurious
    ``wing'' artifact that used to appear in the reconstructed Weiss
    field.
