...
private:
	                    ...
	                    const BasisBaseType& basis) const
	{
		...
		for (SizeType j = 0; j < nsite; ++j) {
			...
			if (hasHop && s1i == 1 && s1j == 0) {
				...
				RealType tmp2 = ProgramGlobals::doSign(ket, i)*ProgramGlobals::doSign(bra1, j);
	                           ...
	                           const LabeledOperatorType& lOperator) const
	{
		...
		if (newPart1 < 0) return false;
		SizeType nsite = geometry_.numberOfSites();
		if (static_cast<SizeType>(newPart1) > nsite) return false;
		if (newPart1==0) return false;
		newParts = newPart1;
		return true;
	}
